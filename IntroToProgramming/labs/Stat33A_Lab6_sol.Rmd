...
predict_grade_new = function(current, desired, weight){
  if(weight < 0 | weight > 1){
    stop('check weight: It needs to be a fraction between 0 and 1')
  }
  (desired-current*(1-weight))/weight
}
...
bmi_unit = function(ht, wt, unit = "in"){
  if ((!unit %in% c("in", "ft", "cm", "m"))) {
    warning("unit value not valid. using inches")
    unit = "in"
  }
  c = 1
  if (unit == "in") { 
...
ht3 = c(152.40, 167.64, 182.88)
wt3 = c(45.35920, 63.50288, 90.71840)
bmi_unit(ht3, wt3, unit = "cm")

ht4 = c(1.5240, 1.6764, 1.8288)
