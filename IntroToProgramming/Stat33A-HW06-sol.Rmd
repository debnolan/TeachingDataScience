...
,
```{r}
bus = read.csv(url("http://www.stat.berkeley.edu/users/nolan/data/businesses.csv"), 
                  encoding='ISO-8859-1', stringsAsFactors = FALSE)
ins = read.csv(url("http://www.stat.berkeley.edu/users/nolan/data/inspections.csv"),
...
ANS: A resrtaurant can have multiple inspections, and an inspection can result in multiple violations.
...
* The key for `bus` is most likely: business_id

* The key for `ins` is most likely: business_id and date
...
```{r}
checkGran(bus["business_id"])  # 6315 6315
```
...
```{r}
checkGran(bus["name"])  # 6315 5692
```

### Q. Could the restaurant name be used as the key? Why or why not?

ANS: No, because there are some restaurants with the same name.

### Q. Even if the restaraunt names are unique, why is it not a good idea to use it as a key?

ANS: An ID number is a simpler key. Names can easily get typos and they can be unnecessarily long.
...
ANS: There are some 0000s, empty strings, and bad values like Ca and CA.
...
```{r}
bus$zip[bus$zip %in% c("CA", "Ca", "", "00000", "941")] = NA
```
...
```{r}
filter(bus, zip %in% c("94545", "94602", "94609"))
```
...
+ 94545 - it's vending machine company with many locations 
+ 94602 - it's probably a typo and should be 94102
+ 94609 - Food truck based in oakland but probably travels around.
...
```{r}
checkGran(ins[c("business_id", "date")])
```
...
ANS: Restaurant 64859 was inspected 5 times. Once in 206, twice in 2015 and twice in 2014. The inspection dates in 2015 are identical, but the scores are not the same. It may be that two inspections wree carried out or that one inspection was accidentally duplicated in the process of updating the record. We don't know.
...
```{r}
partDate = function(dates, part = "year") {
  newDate = as.Date(as.character(dates), "%Y%m%d")
  if (part == "year") {
    newDate = format(newDate, "%Y")
  } else newDate = format(newDate, "%b")
...
```{r}
ins$yr = partDate(ins$date)
ins$mon = partDate(ins$date, part = "month")
```
...
```{r}
ggplot(filter(ins, yr == "2014"), 
       aes(x = factor(mon, ordered = TRUE, 
...
This puts businesses on a more equal footing because [inspection guidelines](https://www.sfdph.org/dph/eh/Food/Inspections.asp) generally refer to how many inspections should occur in a given year.

Call this reduced data frame `ins2016`.

```{r}
ins2016 = filter(ins, yr == 2016)
```
...
```{r}
checkGran(ins2016[c("business_id", "date")], TRUE)
```

### Q. Is there one record per key? 

ANS: No, there is one duplicate key, at the 5703 record in the data frame.
...
```{r}
count(count(ins2016, business_id), n)
```

### Q. How many restaurants had 3 inspections in 2016? Did any have more?

ANS: 83 restaurants had 3 inspections in 2016. None had more than 3.
...
```{r}
ins2016x = group_by(ins2016, business_id)
ins2016grp = summarize(ins2016x, 
                       date = first(date), score = first(score),
                       mon = first(mon), num_ins = n())
```
...
```{r}
checkGran(ins2016grp["business_id"])
```
...
Call this new data frame `ins2016.bus`.

```{r}
ins.bus2016 = inner_join(ins2016grp, 
                select(busR, c("business_id", "name", "address", "zip")), 
                by = "business_id")
```
