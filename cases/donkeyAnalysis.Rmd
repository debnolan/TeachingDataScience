...
The data are available in the the `donkeys` data frame in the `paranomo` package so we load it in with the following call.
...
* `Girth` - circumference behind fron leg (cm)
...
The variable types help us figure out what kinds of summary statistics and plots to make to explore the data. With qualitative data we examine tables of counts or percentages and bar plots.  For quantiative variables we examine quantiles, means and sds, and histograms. Let's examine each variable in turn.
...
```{r}
ggplot(donkeys, aes(x = Length, y = ..density..)) + geom_histogram(bins= 20)
ggplot(donkeys, aes(x = Girth, y = ..density..)) + geom_histogram(bins= 20)
ggplot(donkeys, aes(x = Height, y = ..density..)) + geom_histogram(bins= 20)
```

All three distributions show a skew to the left, i.e., more smaller donkeys. The medians being larger than the means confirm this, but the difference between them is small. Also, wesee that there is an very small value in each of the histogram. 
...
We want to examine both the relationship between the outcome variable and the other variables and between the explanatory variables.  The relationship between the outcome and explanatory variables give us an idea as to how these values vary together for donkeys. The relationship between the eplanatory



```{r}
pairs(donkeys[ , c("Weight", "Length", "Girth", "Height")])
```

The relationships between weight and the other quantitative variables are all highly correlated, with the relationship between girth and weight appearing to be the strongest.  
We see that girth, height and length are also correlated so that any one of these variables captures a lot of the information in the others. 
...
In addition to the quantitative variables, we can also examine the relatinship between weight and the categorical variables. We can compare histograms of weight for the different cateogries, or examine the quartiles and extremes of the subgroups of weight via boxplots.
...
These three subgroups have are similar in median. The IQR for geldings is small in comparison to the others. The geldings may tend to weight more and be less variable in their weights?
...
```{r}
donkeys = mutate(donkeys, Gelding = Sex == "gelding")
```


Next, we examine the distribution of weight for the various body conditions. 
To do this, we need to convert BCS into a factor.
...
*Implication* The weight distribution appears to be shifter upward for each cateogry of body condition so we most likely want to use body condition as a variable and do not want to collapse the categories.  

We also examine the weight distrbituions for the age subgroups. 
...
Notice that the orders of the age grops are all mixed up. This is because they are being plotted in alphabetical order. To fix this, we need to create an ordered factor.

```{r}
donkeys = mutate(donkeys, Age = factor(Age, levels = c("<2", "2-5", "5-10", "10-15",    "15-20", ">20" ), ordered = TRUE))
```

Now we mkae our boxplot again.
...
donkeys = mutate(donkeys, Age2 = Age2)
```

### Transformation

The scatter plots look elliptical, indicating that we may not need any transformations. 
However, the long left tail in the distributions imply that we may want to move up the ladder of transformations, e.g., consider taking the square or cube transformation. We explore the square of girth.

```{r}
ggplot(donkeys, aes(x = Girth^2)) + geom_histogram()
```

```{r}
cor(donkeys$Weight^2, donkeys$Girth^2)
```

The histogram appears somewhat improved. The correlation decreases when we square both girth and weight. 

*Implication* Whether or not there is an improvement seems debatable. We don't necessarily want to make our decisions based only on correlations. Let's leave the variables untransformed for now. 
...
```{r}
library(leaps)
leapsOut = regsubsets(Weight ~ Length + Girth + Height + Age2 + 
                       Gelding + BCS, data = donkeys, nbest = 5, nvmax = 8,
                   method = "exhaustive")
```
...
Notice that this is slightly problematic in that some of the models include the factor variables, but not all of their levels. 

We do note that the best models are:
...
Model selection is an art. We wn't go further into it.
